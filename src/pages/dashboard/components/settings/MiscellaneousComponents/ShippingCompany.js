...
// _mock_
import { _userList, _shippingItems } from 'src/_mock';
// components
...
import ShippingItemsForm from 'src/sections/@dashboard/calendar/setting/ShippingItemsForm';
...
// ----------------------------------------------------------------------
    ...
    const [shippingItems, setshippingItems] = useState([..._shippingItems]);
    ...
    const [selectedshippingItems, setSelectedshippingItems] = useState(null)
    ...
    const AddButton = () => {
        ...
        const TABLE_HEAD = [
            ...
            { id: 'name', label: 'Contact Person', alignRight: false },
            ...
            <Container  maxWidth={themeStretch ? false : 'lg'}>
                <Card mt={3}>
                    <Scrollbar>
                        ...
                        <h4 style={{marginBottom:15, marginTop:10, textAlign:'center', color:'#ff6347', fontSize:30}}> Shipping Company Details</h4>
                            <Table>
                                <UserListHead
                                    order={order}
                                    orderBy={orderBy}
                                    headLabel={TABLE_HEAD}
                                    rowCount={shippingItems.length}
                                    numSelected={selected.length}
                                    onRequestSort={handleRequestSort}
                                    onSelectAllClick={handleSelectAllClick}
                                />
                                <TableBody >
                                    {shippingItems.map((row) => {
                                        const { id, bold, 
                                            shippingname,
                                        ...
                                        const isItemSelected = selected.indexOf(shippingname) !== -1;
                                        return (
                                            <TableRow
                                                hover
                                                key={id}
                                            ...
                                            >
                                                ...
                                                <TableCell align="left">{shippingcontactperson}</TableCell>
                                                <TableCell align="left">{shippingcontactnumber}</TableCell>
                                                <TableCell align="left">{shippingsecondarynumber}</TableCell>
                                                <TableCell align="left">{shippingaddress}</TableCell> 
                                                <TableCell align="right">
                                                <UserMoreMenu onDelete={() => handleDeleteUser(id)} handleEditEvent={() => handleEditEvent(row)} userName={shippingname} />
                                                </TableCell>
                                            </TableRow>
                                        );
                                    })}
                 ...
                 <DialogAnimate modalWidth='sm' open={isOpenModal} onClose={handleCloseModal}>
                    <DialogTitle>{selectedshippingItems ? 'Edit Shipping Company Details' : 'Add Shipping Company'}</DialogTitle>
                    <ShippingItemsForm shippingItems={shippingItems} setshippingItems={setshippingItems} event={selectedshippingItems || {}} range={selectedRange} onCancel={handleCloseModal} />
                </DialogAnimate>
            </Container>
        </Page>
    );
