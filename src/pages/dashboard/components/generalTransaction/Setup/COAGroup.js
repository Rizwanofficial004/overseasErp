...
// _mock_
import { _userList, _coaGroupItems } from 'src/_mock';
// components
...
import COAGroupItemsForm from 'src/sections/@dashboard/calendar/generalledger/COAGroupItemsForm';
...
// ----------------------------------------------------------------------
    ...
    const [coaGroupItems, setcoaGroupItems] = useState([..._coaGroupItems]);
    ...
    const [selectedcoaGroupItems, setSelectedcoaGroupItems] = useState(null)
    ...
    const AddButton = () => {
        ...
        const TABLE_HEAD = [
            { id: 'role', label: ' Group ID   ', alignRight: false },
            { id: 'name', label: 'Group Name', alignRight: false },
            { id: 'company', label: 'SubGroup Of    ', alignRight: false },
            { id: 'company', label: ' Class', alignRight: false },
            ...
            <Container  maxWidth={themeStretch ? false : 'lg'}>
                <Card mt={3}>
                        ...
                        <h4 style={{marginBottom:15, marginTop:10, textAlign:'center', color:'#ff6347', fontSize:30}}>Chart Of Account Details</h4>
                            <Table>
                                <UserListHead
                                    ...
                                    rowCount={coaGroupItems.length}
                                    numSelected={selected.length}
                                    onRequestSort={handleRequestSort}
                                    onSelectAllClick={handleSelectAllClick}
                                />
                                <TableBody >
                                    {coaGroupItems.map((row) => {
                                        const { id, bold, 
                                            groupid,
                                            groupName,
                                            subGroupOf,
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <TableCell align="left">{subGroupOf}</TableCell>
                                                <TableCell align="left">{classes}</TableCell>
                                                
                                                <TableCell align="right">
                                                <UserMoreMenu onDelete={() => handleDeleteUser(id)} handleEditEvent={() => handleEditEvent(row)} userName={groupid} />
                                                </TableCell>
                                            </TableRow>
                                        );
                                    })}
                 ...
                 <DialogAnimate modalWidth='sm' open={isOpenModal} onClose={handleCloseModal}>
                    <DialogTitle>{selectedcoaGroupItems? 'Edit COA Group ' : 'COA Group'}</DialogTitle>
                    <COAGroupItemsForm  coaGroupItems={coaGroupItems} setcoaGroupItems={setcoaGroupItems} event={selectedcoaGroupItems || {}} range={selectedRange} onCancel={handleCloseModal} />
                </DialogAnimate>
            </Container>
        </Page>
    );
