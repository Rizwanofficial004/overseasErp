...
// _mock_
import { _userList, _currencyItems } from 'src/_mock';
// components
...
// ----------------------------------------------------------------------
    ...
    const [currencyItems, setcurrencyItems] = useState([..._currencyItems]);
    ...
    const [selectedcurrencyItems, setSelectedcurrencyItems] = useState(null)
    ...
    const AddButton = () => {
        ...
        const TABLE_HEAD = [
            { id: 'role', label: 'Currency Abbreviation ', alignRight: false },
            { id: 'name', label: 'Currency Symbol', alignRight: false },
            { id: 'company', label: 'Currency Name ', alignRight: false },
            { id: 'company', label: ' Hundredths Name', alignRight: false },
            ...
            <Container  maxWidth={themeStretch ? false : 'lg'}>
                <Card mt={3}>
                    <Scrollbar>
                        ...
                        <h4 style={{marginBottom:15, marginTop:10, textAlign:'center', color:'#ff6347', fontSize:30}}> Currency Details</h4>
                            <Table>
                                <UserListHead
                                    order={order}
                                    orderBy={orderBy}
                                    headLabel={TABLE_HEAD}
                                    rowCount={currencyItems.length}
                                    numSelected={selected.length}
                                    onRequestSort={handleRequestSort}
                                    onSelectAllClick={handleSelectAllClick}
                                />
                                <TableBody >
                                    {currencyItems.map((row) => {
                                        const { id, bold, 
                                        ...
                                        const isItemSelected = selected.indexOf(currencyabbreviation) !== -1;
                                        return (
                                            <TableRow
                                                hover
                                                key={id}
                                            ...
                                            >
                                                ...
                                                <TableCell align="left">{currencysymbol}</TableCell>
                                                <TableCell align="left">{currencyname}</TableCell>
                                                <TableCell align="left">{hundredthsname}</TableCell>
                                                <TableCell align="left">{country}</TableCell> 
                                                <TableCell align="right">
                                                <UserMoreMenu onDelete={() => handleDeleteUser(id)} handleEditEvent={() => handleEditEvent(row)} userName={currencyabbreviation} />
                                                </TableCell>
                                            </TableRow>
                                        );
                                    })}
                 ...
                 <DialogAnimate modalWidth='sm' open={isOpenModal} onClose={handleCloseModal}>
                    <DialogTitle>{selectedcurrencyItems? 'Edit Currency Details' : 'Add Currency'}</DialogTitle>
                    <CurrencyItemsForm currencyItems={currencyItems} setcurrencyItems={setcurrencyItems} event={selectedcurrencyItems || {}} range={selectedRange} onCancel={handleCloseModal} />
                </DialogAnimate>
            </Container>
        </Page>
    );
